<?php 
  ...
  $result1        = $mysqli->query("SELECT kode_encrypt FROM tbl_encrypt 
                    WHERE kode_encrypt='$kode_encrypt'");
  $data1          = $result1->num_rows;

  if ($data1>0){
    //kode encrypt sudah terdaftar
    echo "<script>document.location.href='../?mod=encrypt&pg=form_input_encrypt&kode_encrypt=$kode_encrypt&pesan=$pesan&act=ks'</script>";
  } else {
    ...
    if(@$eror == true){
      ...
      function encodePayload($payload, $image, $width, $height) {
      ...
      if(encodePayload($payload, $image, $width, $height)) { 
        ...
        $result = $mysqli->query("INSERT INTO tbl_encrypt VALUES(null,$id_user,'$kode_encrypt',
                  '$pesan','$ciphertext','$file_simpan')"); 
